...
export function buildAndroidApps(BASE_DIR, RN_VERSION, BENCHMARK_DIR, ERROR_LOG_FILE) {
    ...
    try {
        runCommand(`node ${path.join(BASE_DIR, './platform/android/build-android-app.js')} ${RN_VERSION} ${BENCHMARK_DIR} old`, ERROR_LOG_FILE);
    ...
    try {
        runCommand(`node ${path.join(BASE_DIR, './platform/android/build-android-app.js')} ${RN_VERSION} ${BENCHMARK_DIR} new`, ERROR_LOG_FILE);
...
export function runInstallAndTests(runOldArch = true, runNewArch = true, BENCHMARK_DIR, ERROR_LOG_FILE, BASE_DIR, RN_VERSION, ITERATIONS) {
    ...
    if (runOldArch) {
        ...
        try {
            runCommand(`node ${path.join(BASE_DIR, './platform/android/run-android-tests.js')} ${oldArchMetadataPath} ${oldArchDeviceId}`, ERROR_LOG_FILE);
    ...
    if (runNewArch) {
        ...
        try {
            runCommand(`node ${path.join(BASE_DIR, './platform/android/run-android-tests.js')} ${newArchMetadataPath} ${newArchDeviceId}`, ERROR_LOG_FILE);
...
export function modifyGradleProperties(isNewArch, GRADLE_PROPERTIES, PACKAGE_NAME) {
...
export function buildApp(isNewArch, ANDROID_DIR, BENCHMARK_DIR, ERROR_LOG_FILE, APK_PATH) {
    ...
    try {
        ...
        if (!fs.existsSync(releaseApkPath)) {
            logMessage('ERROR', 'Release APK not found at expected path');
            return false;
        }

        fs.copyFileSync(releaseApkPath, APK_PATH);
        logMessage('INFO', `Release APK copied to ${APK_PATH}`);
        return true;
    } catch (error) {
